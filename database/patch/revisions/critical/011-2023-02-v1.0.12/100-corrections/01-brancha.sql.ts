...
alter table cluster.collector
  drop constraint fk_collector_to_share;
alter table cluster.collector
  add constraint fk_collector_to_share foreign key (collector_share_regclass)
    references cluster.share on update cascade;

alter table cluster.object
  drop constraint fk_objecto_to_share;
alter table cluster.object
  add constraint fk_objecto_to_share foreign key (object_share_regclass)
    references cluster.share on update cascade;

alter table cluster.version
  drop constraint fk_version_to_share;
alter table cluster.version
  add constraint fk_version_to_share foreign key (version_share_regclass)
    references cluster.share on update cascade;


update cluster.share
set share_regclass = 'cluster.branch'
where share_regclass = 'tweeks.branch'
;
