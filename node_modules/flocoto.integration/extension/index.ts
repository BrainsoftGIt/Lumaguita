...
namespace extension{
    ...
    type PropsTypeClass = typeof PropsType;
    ...
    type PropsOptions<T extends PropsType> = {
        meta?: T extends typeof PropsType["OPTIONS"]? {
            ...
            min?:any
            max?:any
        }:T extends typeof PropsType[ "OBJECT" ]? {

        }:T extends typeof PropsType[ "BOOLEAN" ]? {

        }:T extends typeof PropsType[ "ARRAY" ]? {

        }:T extends typeof PropsType[ "FILE" ]? {
            description?: string,
    ...
    export type ProcessResult<T extends Meta<T>, M extends keyof T["methods"]> = {
        ...
        message?:string,
        returns: {
            [P in keyof T["methods"][M]["returns"] ]:
                T["methods"][M]["returns"][P]["type"] extends PropsType.TEXT? string
                :T["methods"][M]["returns"][P]["type"] extends PropsType.NUMBER? number
                :T["methods"][M]["returns"][P]["type"] extends PropsType.BOOLEAN? boolean
                :T["methods"][M]["returns"][P]["type"] extends PropsType.DATE? Date
                :T["methods"][M]["returns"][P]["type"] extends PropsType.OBJECT? { [p:string]:any }
                :T["methods"][M]["returns"][P]["type"] extends PropsType.LIST? any[]
                :T["methods"][M]["returns"][P]["type"] extends PropsType.OPTIONS? any[]
                :T["methods"][M]["returns"][P]["type"] extends PropsType.FILE? {
                    filename:string,
                    extension: string,
                    oldFileName: string,
                    reference:string,
    ...
    export type ExtensionHandlers<T extends Meta<T>, H extends { [ K in keyof T["methods"]]:( context:MetaContext<T, K> )=>ReturnType<H[K]>}> = H

    export function extension <T extends Meta<T>, H extends { [ M in keyof T["methods"]]:( context:MetaContext<T, M> )=>ReturnType<H[M]>}>( meta:T, handler:ExtensionHandlers<T, H>):ExtensionHandlers<T, H>{
        return handler
    }
