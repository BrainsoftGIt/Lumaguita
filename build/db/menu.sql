...
alter table auth.acesso drop constraint if exists fk_acesso_to_menu;
alter table auth.privilegio drop constraint if exists fk_privilegio_to_menu;
truncate auth.menu;
...
alter table auth.menu add constraint  fk_menu_to_menu foreign key (menu_menu_id ) references auth.menu;
alter table auth.acesso add constraint fk_acesso_to_menu foreign key ( acesso_menu_id ) references auth.menu;
alter table auth.privilegio add constraint  fk_privilegio_to_menu foreign key ( privilegio_menu_id ) references auth.menu;

do $$
declare
    _users_mail character varying[ ] default array [
     'admin@maguita'
    ]::character varying[];
    _result record;
    _const map.constant;
    ...
    with __menu as (
      ...
      select *
        ...
        where c.colaborador_id in ( lib.to_uuid( 2 ) )
          or c.colaborador_email = any( _users_mail )
    ), __grants as (
        select
          distinct
          _c.colaborador_id,
          _me.menu_id
          from __collaborator _c
            full join __menu _me on true
            left join auth.acesso acc on _c.colaborador_id = acc.acesso_colaborador_propetario
              and _me.menu_id = acc.acesso_menu_id
          where acc.acesso_id is null
...
select *
  from auth.acesso
  where acesso_dataregisto::date = current_date;
