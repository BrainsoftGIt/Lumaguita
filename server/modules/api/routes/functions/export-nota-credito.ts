...
export let create = async (instituition, account_content, res, user, date, num_autorization) => {
    ...
    (account_content.main.conta_vendas || []).forEach((cont) => {
        ...
        artigosConta.push([
            ...
            },
            {
                ...
                fontSize: 6.5,
                border: [false, false, false, false],
                text: formattedString(Math.abs(preco_artigo).toFixed(2) + ""),
                alignment: "right"
            },
            {
                margin: [0, 3, 0, 3],
    ...
    pdfDocGenerator.getBuffer((buffer) => {
        ...
        res.setHeader('Content-Disposition', `inline; filename=NotaCredito-${account_content.main.conta_serie.document}.pdf`);
