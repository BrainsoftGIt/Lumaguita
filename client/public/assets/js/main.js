...
let Documents = {
    ...
    open : ({data, name}) => {
        ...
        $("[listDocuments]").append(`<li data-key="${key}" class="flex v-ct tgl">
            ...
            <span>${name} <document></document></span>
        </li>`);
    ...
    getDocumentNumber: (key) => {
        ...
        let {origin} = location;
        var url = new URL(urlData, origin);

        // Extract the URL-encoded JSON string from the path
        var encodedJsonString = url.pathname.split("/").pop();
        ...
        if(!jsonData.conta_id && !jsonData.deposito){
            return
        }

        $.ajax({
            url: "/api/get/number/document",
            method: "POST",
