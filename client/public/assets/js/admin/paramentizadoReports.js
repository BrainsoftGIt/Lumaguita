                ...
                list.forEach(({ filter_valuemode, filter_value, filter_column, filter_name, filter_opr, filter_props: { key, format, src, source }, filter_type, filter_mode }) => {
                    ...
                    if (format === "select") {
                       ...
                       opr="${filter_opr}" key="${key}" mode="${filter_mode}">
                                   <input ${PedirSempre !== filter_valuemode.toString() ? " _noObrigatory='true' class='_noObrigatory' " : ""} ${disabledText} value="${((filter_value || "").stringToDateEn() || "").getDatePt()}" data-inputmask-alias="dd-mm-yyyy" data-val="true" type="text"
                       ...
                       opr="${filter_opr} "key="${key}" mode="${filter_mode}">
                                       <input ${PedirSempre !== filter_valuemode.toString() ? " _noObrigatory='true' class='_noObrigatory' " : ""} ${disabledText} value="${filter_value || ""}" type="text" placeholder="${filter_name}">
                                       <label>${filter_name}</label>
                                   </div>`);
                    }
