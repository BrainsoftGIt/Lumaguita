  ...
  loadMenus(){
      $.ajax({
          ...
          success({TAG, ...e}) {
            ...
            menuAcessos.html(`<li data-position="-100" class="tgl flex v-ct active" link="../includes/home?v=${TAG}" title="Home" code="main">
            ...
            colaborador_logado_armazens.empty();
              ...
              menusSemFaturasGuias.forEach((meSFQ) => {
                  ...
                  menuAcessos.append(`<li data-position="${meSFQ.menu_position}" class="tgl flex v-ct" link="${meSFQ.menu_link}?v=${TAG}" title="${meSFQ.menu_nome}" code="${meSFQ.menu_codigo}">
                                            ${meSFQ.menu_icon}
                ...
                acesso.menusFaturasGuias.forEach((meFQ) =>{
                    ...
                    if((meFQ.menu_codigo === "maguita.fatura" || meFQ.menu_codigo === "maguita.proforma" || meFQ.menu_codigo === "maguita.notacredito" || meFQ.menu_codigo === "maguita.documentos")){
                        menuAcessos.append(`<li data-position="${meFQ.menu_position}" class="tgl flex v-ct" link="${meFQ.menu_link}?v=${TAG}" title="Faturas" tipo="fatura" code="${meFQ.menu_codigo}">
                        ...
                        if(menuAcessos.find("li[tipo=guia]").length === 0 && (meFQ.menu_codigo === "maguita.entrada.artigos" || meFQ.menu_codigo === "maguita.saida.artigos")){
                            menuAcessos.append(`<li data-position="${meFQ.menu_position}" class="tgl flex v-ct" link="${meFQ.menu_link}?v=${TAG}" title="Guias" tipo="guia" code="${meFQ.menu_codigo}">
                                            ${meFQ.menu_icon}
                                        <span>Guias</span>
                                    </li>`);
                        }
            ...
            if(acesso.showConfig){
                menuAcessos.append(`<li data-position="1000" class="tgl flex v-ct" link="../includes/systemConfig?v=${TAG}" title="Configurações do sistema" code="sys">  
